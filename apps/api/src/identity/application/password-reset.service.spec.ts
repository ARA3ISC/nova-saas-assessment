...
describe('PasswordResetService', () => {
  it('invalidates existing tokens and creates a new token', async () => {
    ...
    expect(repository.invalidateForIdentity).toHaveBeenCalledWith('identity-id');

    const expiresAt = vi.mocked(repository.create).mock.calls[0]?.[2];
    expect(repository.create).toHaveBeenCalledWith('identity-id', expect.any(String), expiresAt);
    expect(expiresAt!.getTime()).toBeGreaterThanOrEqual(before + 30 * 60 * 1000);
